...
Traceback (most recent call last):
  File {Fore.MAGENTA}"{DirLocation._resolve_path("-<root\\main.py")}"{Style.RESET_ALL}, line {Fore.MAGENTA}232{Style.RESET_ALL}, in {Fore.MAGENTA}<module>{Style.RESET_ALL}
    {Fore.LIGHTRED_EX}run{Fore.RED}(){Style.RESET_ALL}
    {Fore.LIGHTRED_EX}~~~{Fore.RED}^^{Style.RESET_ALL}
  File {Fore.MAGENTA}"{DirLocation._resolve_path("-<es\\dirloct.py")}"{Style.RESET_ALL}, line {Fore.MAGENTA}98{Style.RESET_ALL}, in {Fore.MAGENTA}<module>{Style.RESET_ALL}
    {Fore.LIGHTRED_EX}tf u mean, im not {Fore.RED}the error!{Style.RESET_ALL}
    {Fore.LIGHTRED_EX}~~~~~~~~~~~~~~~~~~{Fore.RED}^^^^^^^^^^{Style.RESET_ALL}
{Fore.LIGHTMAGENTA_EX}{err}{Style.RESET_ALL}
      """)
    time.sleep(1)
...
class AppOper:
    ...
    def restart_app():
        ...
        try:
            clr()
            from .dirloct import DirLocation as Dl
            subprocess.run(Dl._resolve_path("..\\..\\run.bat"), check=False)
            sys.exit(0)
        except (FileNotFoundError, subprocess.CalledProcessError): sys.exit(0)
    ...
    def uninstaller():
        ...
        easysaxo.k_log("a0")
        with open(note_path, "w", encoding="utf-8") as f: f.write("Thanks for using EasySaxo! I'll miss you. :(\n")
        ...
        for file in FileList._delete: # delete app files
            ...
            if os.path.exists(file_path):
                ...
                except PermissionError: """Just ignore the file."""
