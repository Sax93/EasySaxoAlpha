...
@register_command("python", aliases=["py", "pydata"], registry=GET_REGISTRY, help_text="get python - Displays Python version and path info.")  # pure hatred!
...
def t_web(arg):
    ...
    if "-d" in parts:
        ...
        # check if a filename stands after -d
        if d_idx + 1 < len(parts) and not parts[d_idx + 1].startswith("-"):
            download_path = parts[d_idx + 1]
            parts.pop(d_idx + 1)
